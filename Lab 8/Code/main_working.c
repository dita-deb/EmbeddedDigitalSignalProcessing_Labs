...
float32_t input_signal[FFT_LENGTH];//export 8192

float32_t cos_signal[FFT_LENGTH];
float32_t new_input[FFT_LENGTH];

float32_t output_fft[FFT_LENGTH]; //export 8192
float32_t output_fft_mag[FFT_LENGTH/2]; //export 4096
float32_t output_freq[FFT_LENGTH/2]; //export 4096
...
* 0 Hz - 600 Hz
  gain = 1
  desired ripple = 1 dB
  actual ripple = 0.5243094188981824 dB

* 1500 Hz - 8192 Hz
  gain = 0
  desired attenuation = -40 dB
  actual attenuation = -43.13018286656774 dB

*/


static float32_t Coeffs[32] = {
  -0.004344147732779473,
  -0.009168771306895345,
  -0.009724726663831892,
  -0.014577553983187346,
  -0.014952323254611453,
  -0.014925558361240522,
  -0.01026561726418485,
  -0.0024817460791642034,
  0.010003390455751927,
  0.025902379839063516,
  0.04482221705027414,
  0.06486658161749063,
  0.08440325053709288,
  0.10127458250446596,
...
int main(void)
{
  ...
  for (int i=0; i<FFT_LENGTH; i++)
  {
	  ...
	  //new_input[i] = arm_cos_f32(2*PI*400*i/SAMPLING_RATE) + arm_cos_f32(2*PI*2000* i/SAMPLING_RATE);
  ...
  //arm_rfft_fast_f32(&fft_handler, new_input, output_fft,0);
